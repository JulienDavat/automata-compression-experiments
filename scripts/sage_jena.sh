...
if [ "$#" -ne 5 ]; then
  ...
  echo "Usage: ./sage_jena.sh <server> <default-graph> <queries-directory> <output-folder> <query-timeout>"
  exit 1
fi
...
mkdir -p ${OUTPUT}/results/jena_sage/ 1> /dev/null 2>&1
mkdir -p ${OUTPUT}/temp/ 1> /dev/null 2>&1

RESFILE_TEMP="${OUTPUT}/temp/jena_sage_metrics.csv"
RESFILE="${OUTPUT}/jena_sage_metrics.csv"
...
for i in $(seq $START $END); do
    ...
    for qfile in ${QUERIES}/*.sparql; do
        ...
        if [ "$i" -eq 0 ]; then
            ...
            sage-jena $DEFAULT_GRAPH --file $qfile --timeout $QUERY_TIMEOUT
        else
            # warm evaluation of the query (result and statistics are retrieved)
            echo -n "${qname}," >> $RESFILE_TEMP
            
            sage-jena $DEFAULT_GRAPH --file $qfile --timeout $QUERY_TIMEOUT --format json --output ${OUTPUT}/results/jena_sage/${qname}.json --measure ${OUTPUT}/results/jena_sage/${qname}.csv

            if [ -f ${OUTPUT}/results/jena_sage/${qname}.csv ]; then
                time=$(cat ${OUTPUT}/results/jena_sage/${qname}.csv | awk -F',' '{printf "%s", $1}' | tr -d '"')
                httpCalls=$(cat ${OUTPUT}/results/jena_sage/${qname}.csv | awk -F',' '{printf "%s", $2}' | tr -d '"')
                dataTransfer=$(cat ${OUTPUT}/results/jena_sage/${qname}.csv | awk -F',' '{printf "%s", $4}' | tr -d '"')
                state=$(cat ${OUTPUT}/results/jena_sage/${qname}.csv | awk -F',' '{printf "%s", $12}' | tr -d '"')
                nbResults=0
                if [ "${state}" == "complete" ] && [ -f ${OUTPUT}/results/jena_sage/${qname}.json ]; then
                    nbResults=$(jq '.results.bindings | length' ${OUTPUT}/results/jena_sage/${qname}.json)
                fi
