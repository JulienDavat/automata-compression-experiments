...
if [ "$#" -ne 5 ]; then
  ...
  echo "Usage: ./sage_multi_automaton.sh <server> <default-graph> <queries-directory> <output-folder> <query-timeout>"
  exit 1
fi
...
mkdir -p ${OUTPUT}/results/sage_multi_automaton 1> /dev/null 2>&1
mkdir -p ${OUTPUT}/temp/ 1> /dev/null 2>&1

RESFILE_TEMP="${OUTPUT}/temp/sage_multi_automaton_metrics.csv"
RESFILE="${OUTPUT}/sage_multi_automaton_metrics.csv"
...
for i in $(seq $START $END); do
    ...
    for qfile in ${QUERIES}/*.sparql; do
        ...
        if [ "$i" -eq 0 ]; then
            ...
            node --max-old-space-size=16384 ../node_modules/.bin/multi-sage-select $SERVER $DEFAULT_GRAPH --file $qfile --timeout $QUERY_TIMEOUT
        else
            echo -n "${qname}," >> $RESFILE_TEMP

            # warm evaluation of the query (result and statistics are retrieved)
            node --max-old-space-size=16384 ../node_modules/.bin/multi-sage-select $SERVER $DEFAULT_GRAPH --file $qfile --timeout $QUERY_TIMEOUT --output ${OUTPUT}/results/sage_multi_automaton/${qname}.json --measure ${OUTPUT}/results/sage_multi_automaton/${qname}.csv

            if [ -f ${OUTPUT}/results/sage_multi_automaton/${qname}.csv ]; then
                time=$(cat ${OUTPUT}/results/sage_multi_automaton/${qname}.csv | awk -F',' '{printf "%s", $1}' | tr -d '"')
                httpCalls=$(cat ${OUTPUT}/results/sage_multi_automaton/${qname}.csv | awk -F',' '{printf "%s", $2}' | tr -d '"')
                dataTransfer=$(cat ${OUTPUT}/results/sage_multi_automaton/${qname}.csv | awk -F',' '{printf "%s", $3}' | tr -d '"')
                nbResults=$(cat ${OUTPUT}/results/sage_multi_automaton/${qname}.csv | awk -F',' '{printf "%s", $4}' | tr -d '"')
                state=$(cat ${OUTPUT}/results/sage_multi_automaton/${qname}.csv | awk -F',' '{printf "%s", $5}' | tr -d '"')
